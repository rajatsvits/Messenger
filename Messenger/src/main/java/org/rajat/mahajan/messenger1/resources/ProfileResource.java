package org.rajat.mahajan.messenger1.resources;

import java.util.List;

import javax.ws.rs.Consumes;
import javax.ws.rs.DELETE;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.PUT;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;
import javax.ws.rs.core.MediaType;


import org.rajat.mahajan.messenger1.model.Profile;
import org.rajat.mahajan.messenger1.service.ProfileService;

@Path("/profiles")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class ProfileResource {

ProfileService profileService = new ProfileService();
Profile profile = new Profile();
	
	@GET
	public List<Profile> getAllProfiles() {
		return profileService.getAllProfiles();
	}
	
//	@GET
//	@Path("{messageId}")
//	public Profile getProfile(@PathParam("messageId") long id) {
//		String profileName = profile.getProfileName();
//		return profileService.getProfile(profileName);
//	}
	
	@GET
	@Path("{profileName}")
	public Profile getProfile(@PathParam("profileName") String pN) {
		return profileService.getProfile(pN);
	}
	
	@POST
	public Profile addProfile(Profile profile) {
		return profileService.addProfile(profile);
		
	}
	
	@PUT
	@Path("{profileName}")
	public Profile updateProfile(@PathParam("profileName") String profileName ) {
		Profile profile = profileService.getProfile(profileName);
		profileService.updateProfile(profile);
		return profile;
	}
	
	@DELETE
	@Path("{profileName}")
	public void deleteProfile(@PathParam("profileName") String profileName) {
		profileService.DeleteProfile(profileName);
	}
}
